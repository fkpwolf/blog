...
要求 Linux 4.14+, kvm，只模拟virtio-net, virtio-block, serial console, and a 1-button keyboard controller used only to stop the microVM， 所以速度快。Kata 在 QEMU 基础上实现了容器接口(OCI)，这个只是一个更快速的 QEMU。AWS Lambda，更安全，速度更快(125ms 启动)。
