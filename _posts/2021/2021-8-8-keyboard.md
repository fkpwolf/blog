...
* 试过几个键盘，2.4G 唤醒后会出现第一个按键丢失的问题，根源在于太短时间无按键后就进入休眠，而这个时候可能 Windows 都还没有启动屏幕休眠。这种细节问题在成品键盘上比如 IKBC 的就不会出现。蓝牙则会在 10 分钟后自动休眠，这个时间没法定制也没法关闭，而休眠后重新配对比2.4G则长很多，如果电脑处于休闲模式，比如看会视频后再操作键盘，这种频繁的配对非常影响体验。
