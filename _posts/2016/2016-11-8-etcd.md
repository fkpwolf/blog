...
一次重启 vm 后，operator 创建的 pod 没有启动，状态为『已结束：Completed』，operator pod 有日志：`level=warning msg="all etcd pods are dead."` 这里 [issue](https://github.com/coreos/etcd-operator/issues/1840) 说是已知问题，因为数据没有序列化。
